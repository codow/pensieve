...
export class MeshGraph implements MeshGraphArea {
  ...
  init(obstacles: Array<MeshGraphArea> = []) {
    ...
    for (let i = 1; i < yArray.length; i++) {
      ...
      (row.y1 = yArray[i - 1]), (row.y2 = yArray[i]);
      rows.push(row);
    }
    // 切割行单元格
    obstacles.forEach((item) => {
      ...
      rows = rows.map((row) => {
        let rowY1 = row.y1;
        let rowY2 = row.y2;
        if (rowY1 >= item.y1 && rowY2 <= item.y2) {
          // 切割
          row = flatten(
            row.map((cell) => {
              // 判断哪些单元格需要切割
              if (
                (cell.x1 <= item.x1 && cell.x2 >= item.x1) ||
                (cell.x1 <= item.x2 && cell.x2 >= item.x2)
              ) {
                let cells = [];
                if (cell.x1 < item.x1) {
                  cells.push({
                ...
                cells.push({
                  ...cell,
                  x1: Math.max(item.x1, cell.x1),
                  x2: Math.min(item.x2, cell.x2),
                  passable: false,
                });
                if (cell.x2 > item.x2) {
                  cells.push({
 ...
 * @returns 合并区域
 */
export const mergeArea = function (
  ...areas: Array<MeshGraphArea>
): MeshGraphArea {
  if (!areas.length) {
    return null;
  }
