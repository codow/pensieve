<template>
  ...
  <table class="table-layout"
    ...
    <!-- <tr v-for="(row, rowIndex) of cellModels"
        ...
        :class="{[`row-${rowIndex}`]: true}">
      <template v-for="(cell, colIndex) of row">
        <table-col-cell v-if="rowIndex === 0 && colIndex !== 0"
                        :key="cell.uid"
                        :model="cell"
                        :row-index="rowIndex"
                        :col-index="colIndex"
        ...
        <table-row-cell v-else-if="rowIndex !== 0 && colIndex === 0"
                        :key="cell.uid"
                        :model="cell"
                        :row-index="rowIndex"
                        :col-index="colIndex"
        ...
        <table-col-cell v-else-if="rowIndex === 0 && colIndex === 0"
                        :key="cell.uid"
                        :model="cell"
                        :row-index="rowIndex"
                        :col-index="colIndex"
    ...
    <!-- 选择框上边线 -->
    <div ref="topLine"
         class="select-box__line select-box__top-line"></div>
    <!-- 选择框右边线 -->
    <div ref="rightLine"
         class="select-box__line select-box__right-line"></div>
    <!-- 选择框下边线 -->
    <div ref="bottomLine"
         class="select-box__line select-box__bottom-line"></div>
    <!-- 选择框左边线 -->
    <div ref="leftLine"
         class="select-box__line select-box__left-line"></div>
...
export default {
  ...
  methods: {
    ...
    handleRowResizeLineMousedown ($event) {
      ...
      // 动态添加全局的拖动事件
      this.$refs.table.addEventListener('mousemove', this.resizeMouseMoveListener)
      this.$refs.table.addEventListener('mouseup', this.resizeMouseUpListener)
    },
    ...
    resizeMouseUpListener ($event) {
      ...
      this.$refs.table.style.cursor = ''
      // 清理事件
      this.$refs.table.removeEventListener('mousemove', this.resizeMouseMoveListener)
      this.$refs.table.removeEventListener('mouseup', this.resizeMouseUpListener)
      // 改变行列大小
      if (this.resizeModel.type === 'row') {
    ...
    showSelectBox () {
      ...
      let tableTop = tableEl.offsetTop
      let tableLeft = tableEl.offsetLeft
      ...
      let rows = []
      let cols = []
      let tempCell, tempCellEl, tempPostion
      for (let i = startRowIndex, l = endRowIndex + 1; i < l; i++) {
        for (let m = startColIndex, n = endColIndex + 1; m < n; m++) {
          // 获取单元格
          tempCell = this.cellModels[i][m]
          if (tempCell.merged) {
            tempPostion = this.getMergedCellSrcPosition(i, m)
            tempCell = this.cellModels[tempPostion.rowIndex][tempPostion.colIndex]
            tempCellEl = tableEl.querySelector(`.cell-${tempPostion.rowIndex}-${tempPostion.colIndex}`)
          } else {
            tempCellEl = tableEl.querySelector(`.cell-${i}-${m}`)
          }

          // 如果定位未计算
          if (top === -1000 && i === startRowIndex) {
            // 如果上边距定位没有计算，并且当前单元格是第一列
            top = tableTop + tempCellEl.offsetTop
          }
          if (left === -1000 && m === startColIndex) {
            // 如果左边距定位没有计算，并且当前单元格是第一列
            left = tableLeft + tempCellEl.offsetLeft
          }
          // 计算宽高
          if (rows.indexOf(i) === -1) {
            // 未计算的行，则计算高度
            height += tempCellEl.offsetHeight
            for (let j = i, k = i + tempCell.rowSpan; j < k; j++) {
              rows.push(j)
            }
          }
          if (cols.indexOf(m) === -1) {
            // 未计算的列，则计算宽度
            width += tempCellEl.offsetWidth
            for (let j = m, k = m + tempCell.colSpan; j < k; j++) {
              cols.push(j)
            }
          }
        }
        ...
        this.$refs.topLine.style.width = width + borderWidth * 2 + 'px'
        ...
        this.$refs.rightLine.style.height = height + borderWidth * 2 + 'px'
        ...
        this.$refs.bottomLine.style.width = width + borderWidth * 2 + 'px'
        ...
        this.$refs.leftLine.style.height = height + borderWidth * 2 + 'px'
    ...
    showResizeBox (area) {
      ...
      for (let i = startRowIndex, l = endRowIndex + 1; i < l; i++) {
        ...
        this.$refs.resizeTopLine.style.width = width + borderWidth * 2 + 'px'
        ...
        this.$refs.resizeRightLine.style.height = height + borderWidth * 2 + 'px'
        ...
        this.$refs.resizeBottomLine.style.width = width + borderWidth * 2 + 'px'
        ...
        this.$refs.resizeLeftLine.style.height = height + borderWidth * 2 + 'px'
