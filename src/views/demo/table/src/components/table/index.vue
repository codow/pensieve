    ...
    <tr v-for="y of layoutRows"
      ...
      <template v-for="x of layoutCols">
        <td v-if="y === 0 && x !== 0"
            :key="x"
            class="column-header"
            :class="{selected: cellDatas[y][x].isSelected, 'selected-all': cellDatas[y][x].isSelectedAll}"
        ...
        <table-cell v-else
                    :key="x"
                    :model="cellDatas[y][x]"
                    @select-cell="selectCell"></table-cell>

        <!-- <td v-else-if="!cellDatas[y][x].isIgnore"
            :key="x"
            :class="{'selected': cellDatas[y][x].isSelected, 'selected-original': cellDatas[y][x].isOriginallySelected, 'selected-area-top': cellDatas[y][x].isSelectedAreaTopSide, 'selected-area-bottom': cellDatas[y][x].isSelectedAreaBottomSide, 'selected-area-left': cellDatas[y][x].isSelectedAreaLeftSide, 'selected-area-right': cellDatas[y][x].isSelectedAreaRightSide}"
            :row="y"
            :col="x"
            :colspan="cellDatas[y][x].colSpan"
            :rowspan="cellDatas[y][x].rowSpan"
            @mousedown="selectCell(x, y, $event)"
            @mouseenter="contSelectCell(x, y)"
            @mouseleave="clearSelectedText"
            @contextmenu="showCellMenu(x, y, $event)">
          <VueDraggable v-model="cellDatas[y][x].children"
                        :group="{ name: 'tools'}"
                        style="height: 100%; width: 100%;">
            <component v-for="(item, index) of cellDatas[y][x].children"
                       :key="index"
                       :is="item.value"></component>
          </VueDraggable>
        </td> -->

        <!-- <VueDraggable v-else-if="!checkIgnoreCell(x, y)"
                      :key="x"
                      :class="calcCellClass(x, y)"
                      tag="td"
                      v-model="cellDatas[y][x]"
                      :group="{ name: 'tools'}">
...
export default {
  ...
  methods: {
    init () {
      ...
      for (let i = 0; i < rowL; i++) {
        ...
        for (let j = 0; j < colL; j++) {
          ...
          cellDatas[i][j] = new Cell({
            x: j,
            y: i,
            isSelected: (j === 3 && i === 3) ? 1 : 0,
            isOriginallySelected: (j === 3 && i === 3) ? 1 : 0
          })

          cellDatas[i][j].children = []
        }
      }
    ...
    selectCell (x, y, $event) {
      ...
      // 删除老数据的状态
      let oldCell = this.getCell(this.selectedOriginalCell.x, this.selectedOriginalCell.y)
      if (oldCell) {
        oldCell.isSelected = 0
        oldCell.isOriginallySelected = 0
        oldCell.isSelectedAreaTopSide = 0
        oldCell.isSelectedAreaBottomSide = 0
        oldCell.isSelectedAreaLeftSide = 0
        oldCell.isSelectedAreaRightSide = 0
      }
      // 获取cell对象，设置对象选中状态
      let cell = this.cellDatas[y][x]
      cell.isSelected = 1
      cell.isOriginallySelected = 1
      cell.isSelectedAreaTopSide = 1
      cell.isSelectedAreaBottomSide = 1
      cell.isSelectedAreaLeftSide = 1
      cell.isSelectedAreaRightSide = 1
