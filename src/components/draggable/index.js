...
function PlainDragPlugin() {
  let lastActiveSortable
  let lastPlainDropContainer
  let lastPlainDropEl
  let lastPlainDropDirection
  let plainDropEl
  let plainDragImage
  let oldIndex = 0
  let newIndex = 0
  ...
  function createPlainDropEl() {
    ...
    el.innerHTML =
      '<div class="sortable-drop-line__dot"></div><div class="sortable-drop-line__line"></div>'
    return el
  }
  ...
  PlainDrag.prototype = {
    ...
    dragOverValid: function dragOverValid(_ref2) {
      ...
      let prevPlainDropContainer = lastPlainDropContainer
      if (target === el) {
      ...
      if (
        lastPlainDropContainer !== prevPlainDropContainer ||
        !lastPlainDropContainer.contains(plainDropEl)
      ) {
        //
        ...
        if (lastPlainDropContainer.contains(_ref2.dragEl)) {
          ...
          if (lastPlainDropDirection !== direction) {
            ...
            newIndex = insertIndex(
              lastPlainDropEl,
              lastActiveSortable.options.draggable,
              lastPlainDropDirection,
              [Sortable.clone, plainDropEl]
            )
            //  设置方向样式
            plainDropEl.className =
              'sortable-drop-line ' + 'sortable-drop-line--' + direction
            //  获取目标元素的样式，判断插入元素是否影响布局
            this.setPlainDropElStyle(
              getOffsetStyle(
                _ref2.dragEl,
                plainDropEl,
                _ref2.dragEl.parentNode,
                direction
              )
            )
          }
        ...
        if (
          lastPlainDropContainer &&
          lastPlainDropEl &&
          !lastPlainDropContainer.contains(lastPlainDropEl)
        ) {
          lastPlainDropEl = null
        }
      } else if (target !== el && target !== plainDropEl) {
        let prevPlainDropEl = lastPlainDropEl
        ...
        if (onMove(target) !== false) {
          ...
          if (lastPlainDropDirection !== direction) {
            ...
            //  获取目标元素的样式，判断插入元素是否影响布局
            this.setPlainDropElStyle(
              getOffsetStyle(target, plainDropEl, target.parentNode, direction)
            )
          }
        } else {
          lastPlainDropEl = null
        ...
        if (
          prevPlainDropEl &&
          prevPlainDropEl !== lastPlainDropEl &&
          prevPlainDropEl
        ) {
    ...
    drop: function drop(_ref3) {
      let activeSortable = _ref3.activeSortable,
        putSortable = _ref3.putSortable,
        dragEl = _ref3.dragEl
      let toSortable = putSortable || this.sortable
      let options = this.options
      lastPlainDropEl &&
        toggleClass(lastPlainDropEl, options.plainOverClass, false)
      ...
      if (options.plain || (putSortable && putSortable.options.plain)) {
        if (dragEl !== lastPlainDropEl) {
          ...
          if (toSortable !== activeSortable) {
            activeSortable && activeSortable.captureAnimationState()
          }
          if (lastPlainDropEl) {
          ...
          if (toSortable !== activeSortable) {
            activeSortable && activeSortable.animateAll()
          }
        }
      }
    },
...
function getOffsetStyle(target, locEl, container, direction) {
  ...
  let offset = Object.assign(getOffsetPosition(locEl, container), {
