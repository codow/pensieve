...
CodeMirror
  .defineMode('freemarkermixed', function (config) {
    ...
    var regLeftArray = []
    var htmlHasLeftDelimeterRegArray = []

    for (var freemarkerFlag of freemarkerFlagArray) {
      regLeftArray.push(new RegExp('.*' + regEsc(settings.leftDelimiter + freemarkerFlag)))
      htmlHasLeftDelimeterRegArray.push(new RegExp('[^<>]*' + regEsc(settings.leftDelimiter + freemarkerFlag)))
    }
    ...
    var helpers = {
      ...
      cleanChain: function (stream, state, parser) {
        ...
        return (typeof parser === 'string') ? (parser || null) : parser(stream, state)
      },
    ...
    return {
      ...
      indent: function (state, textAfter) {
        if (state.localMode === freemarkerMode || (state.noparse && !state.localMode)) {
          for (var regLeft of regLeftArray) {
            if (regLeft.test(textAfter)) {
